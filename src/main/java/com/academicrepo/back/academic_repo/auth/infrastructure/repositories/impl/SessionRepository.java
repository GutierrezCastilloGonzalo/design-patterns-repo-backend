...
public class SessionRepository implements ISessionRepository {
    ...
    public Optional<DSession> findByToken(String token) {
        return jpaRepository
                .findByTokenAndIsRevokedFalseAndIsActiveTrue(token)
                .map(mapper::toDomain);
    }
    ...
    public Optional<DSession> findByRefreshToken(String refreshToken) {
        return jpaRepository
                .findByRefreshTokenAndIsRevokedFalseAndIsActiveTrue(refreshToken)
                .map(mapper::toDomain);
    }

    @Override
    public List<DSession> findActiveSessionsByUserId(Long userId) {
        return jpaRepository.findByUserIdAndIsRevokedFalseAndIsActiveTrue(userId).stream()
                .map(mapper::toDomain)
